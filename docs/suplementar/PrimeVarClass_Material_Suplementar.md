...
| **PrimeVarClass (domínio + ESM-2)** | **0,907** | 0,864–0,943 | n/a |
...
| REVEL | 0,930 | n/a |
| AlphaMissense | 0,926 | n/a |
| CADD | 0,920 | n/a |
| PrimeVarClass (domínio + ESM-2) | 0,907 | n/a |
...
**Figura S4.** Probabilidade do modelo *versus* escore funcional experimental, por ensaio de DMS, após o alinhamento de coordenadas. A patogenicidade prevista é negativamente associada à função medida nos dois genes (BRCA1: SGE de Findlay ρ = −0,35, AUC 0,795; HDR de Starita ρ = −0,19, AUC 0,712; BRCA2: HDR em VC-8/Hu ρ = −0,61, AUC 0,874).
...
**Retratos de alta resolução (destaque visual).** As figuras a seguir renderizam o mesmo resultado com qualidade de capa (PyMOL open-source, oclusão de ambiente e ray-tracing), servindo como resumo gráfico do trabalho. A escala de cor (de índigo a dourado) mede a intensidade de detecção por resíduo.
...
| **BRCA1 BRCT** (1T29) | 1.092 | 304 (bolso de fosfopeptídeo) | n/a | 73 |
...
**Figura S12b.** AUC-ROC do carro-chefe (domínio + ESM-2) sob CV bloqueada por posição, IC95% por *bootstrap*: **MSH6 0,908** [0,853–0,953], **MSH2 0,885** [0,845–0,920], **RET 0,843** [0,764–0,915], **MLH1 0,794** [0,702–0,878] e **VHL 0,722** [0,560–0,862]. O componente **ESM-2 é o sinal transferível**: eleva os cinco genes, de **+0,055** (MSH2) a **+0,189** (MSH6). Já a consciência de domínio, curada para a arquitetura modular RING/BRCT/DBD de BRCA/TP53, contribui pouco e de forma **desigual** fora dela, de −0,002 (RET) a +0,045 (MSH6): um limite claro que separa o sinal profundo, que generaliza, do prior estrutural manual, específico de arquitetura. As amostras menores (VHL n = 86; RET n = 110) têm IC largos, reportados sem maquiagem. Os rótulos vêm de uma consulta versionada ao ClinVar (`scratch/fetch_clinvar_expanded_panel.py`, 989 variantes definitivas).

**Por que o VHL é o mais fraco (e por que ele fica no painel).** O ClinVar cataloga 447 variantes *missense* de VHL, mas apenas **86** têm classificação definitiva; destas, somente **15 são benignas**. A AUC de 0,722 é estimada sobre esses 15 negativos, o que explica o intervalo de 0,560 a 0,862. A escassez tem causa biológica, não amostral: com 213 aminoácidos e um dobramento compacto cuja função depende da montagem com as elonginas B e C, o VHL tolera pouca substituição no núcleo, e variantes benignas são intrinsecamente raras (80% das *missense* catalogadas permanecem VUS ou conflitantes). Sob 82,6% de positivos, a AUC também perde resolução: a linha de base trivial de AUPRC já é 0,826. A leitura correta é **sinal real, medido com baixa precisão**, e não falha do método no gene. Mantivemos o VHL no painel porque omitir justamente o resultado mais fraco seria seleção; as conclusões de generalização apoiam-se nos genes com amostra adequada, com destaque para o MSH2 (n = 427). Enriquecer a classe benigna por frequência populacional (critérios BA1 e BS1 do ACMG) estreitaria o intervalo e é o próximo passo natural, deliberadamente não executado aqui para não alterar a regra de rotulagem de um único gene do painel.
