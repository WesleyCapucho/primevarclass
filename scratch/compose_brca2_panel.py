"""Assemble the six confirmed-pathogenic BRCA2 panels into one marketing figure:
'O algoritmo também captura mutações em BRCA2'. Mirrors compose_detected_panel.py
(same badge/strip system) but reads scratch/brca2_panel.json.

Run: python scratch/compose_brca2_panel.py
"""
...
TILES = json.load(open("scratch/brca2_panel.json", encoding="utf-8"))
...
def make_tile(t, tw, strip):
    fg = Image.open(os.path.join(SRC, f"b2_{t['name']}.png")).convert("RGBA")
    ...
    d.text((x + 2, int(tw * 0.045) + int(tw * 0.072)), t["dom"],
           font=font(int(tw * 0.034), bold=False), fill=(150, 200, 255))

    fb = font(int(tw * 0.039), bold=True)
...
def build():
    ...
    d.text((gap + int(W * 0.008), int(band * 0.17)),
           "O algoritmo também captura mutações em BRCA2",
           font=font(int(W * 0.030)), fill=(248, 249, 252))
    d.text((gap + int(W * 0.009), int(band * 0.17) + int(W * 0.036)),
           "Seis variantes do domínio de ligação ao DNA (DBD) confirmadas no ClinVar "
           "— todas detectadas pelo PrimeVarClass",
    ...
    for out in OUTS:
        ...
        canvas.save(os.path.join(out, "fig_detected_panel_brca2.png"), quality=95)
    print("saved fig_detected_panel_brca2.png", canvas.size)
