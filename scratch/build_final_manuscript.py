...
def build_resultados(doc):
    ...
    para(doc, "Cada variante foi decomposta pelo mecanismo estrutural afetado e cruzada "
              ...
              "medida experimentalmente, independente do ClinVar: no mapa de saturação "
              "por edição genômica (SGE) do BRCA1, padrão-ouro funcional (FINDLAY et "
              "al., 2018), separa variantes com e sem perda de função com AUC de 0,795 "
              "em 2.140 variantes (0,712 no ensaio HDR de Starita, n = 2.749). Isso é "
              "decisivo para o BRCA1: onde a coorte externa mais fraca continha apenas "
              "21 positivos ruidosos (0,651), um mapa funcional de milhares de variantes "
              "confirma que a competência do modelo é real — a estimativa baixa reflete "
              "ruído de coorte pequena, não falha do método. É uma validação ortogonal "
              "contra fenótipo molecular medido em bancada, não contra outro rótulo in "
              "silico. Entre "
    ...
    para(doc, "A generalização além de BRCA foi testada no TP53, cujas patogênicas se "
              ...
              "novo o ganho de BRCA. A fronteira é "
              "delimitada com honestidade — em genes de patogenicidade truncante "
              "(PALB2, CHEK2) ou difusa (ATM), o sinal missense-domínio é fraco; num "
              "serviço público de oncogenética de Minas Gerais, as VUS de uma coorte "
              "de 210 pacientes do SUS concentraram-se justamente no ATM (7/35; "
              "RIBEIRO et al., 2025) — o gene onde o próprio método reconhece seu "
              "limite, e onde uma triagem calibrada mais faz falta. Por "
    ...
    para(doc, "A validação concentrou-se em BRCA1/BRCA2; as fronteiras de domínio são "
              ...
              "publicados — validação ortogonal contra função medida (SGE de Findlay, "
              "AUC 0,795; HDR de Starita, 0,712), não em novo experimento de bancada "
              "conduzido no âmbito deste trabalho. O sistema apoia pesquisa; não substitui aconselhamento "
