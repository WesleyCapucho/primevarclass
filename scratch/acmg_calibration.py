...
for lvl, lr in ACMG_PATHOGENIC_LR_THRESHOLDS.items():
    ...
    ax.text(0.012, lr * 1.03, f"PP3 {lvl} (LR≥{lr})", fontsize=12, color="#555")
    t = path_thr.get(lvl)
    if t is not None:
        ax.axvline(t, ls=":", color="#c0392b", lw=1)
ax.set_yscale("log")
ax.set_xlabel("Escore do modelo-carro-chefe (probabilidade)", fontsize=14.3)
ax.set_ylabel("Razão de verossimilhança local (LR+)", fontsize=14.3)
